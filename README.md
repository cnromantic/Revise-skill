...
Revise 是一个面向中文本科毕业论文（设计）的通用 AI Skill，用于论文评审、保守修订、Word/PDF 批注和结构化修改说明生成。它尤其适合软件工程类“系统设计与实现”论文，可检查论文结构、语言表达、图表编号、测试说明、参考文献引用关系，以及需求、设计、实现、测试之间的一致性。
...
- 对本科毕业论文（设计）进行初评打分，并给出逐项依据。
- 检查题目、摘要、关键词、英文题名、目录、章节、正文、图表、测试、参考文献等关键部分。
- 对软件工程类论文额外检查需求分析、系统设计、数据库、接口、实现、测试、截图和技术路线一致性。
